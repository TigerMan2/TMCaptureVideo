...
@interface TMCaptureWriterInput ()
...
@property (nonatomic, assign) TMCaptureAudioSessionType  outputFileType; //写入输出文件类型 默认 TMCaptureAudioSessionTypeVideo

@end

@implementation TMCaptureWriterInput
...
- (void)setOutputFileURL:(NSURL *)outputFileURL {
    ...
    if (self.outputFileType == TMCaptureAudioSessionTypeAudio) {
        _assetWriter = [AVAssetWriter assetWriterWithURL:outputFileURL fileType:AVFileTypeAC3 error:nil];
    } else if (self.outputFileType == TMCaptureAudioSessionTypeVideo || self.outputFileType == TMCaptureAudioSessionTypeSilentVideo) {
...
- (void)startWritingToOutputFileAtPath:(NSString *)path fileType:(TMCaptureAudioSessionType)fileType deviceOrientation:(UIDeviceOrientation)deviceOrientation{
    ...
    if (self.outputFileType == TMCaptureAudioSessionTypeSilentVideo || self.outputFileType == TMCaptureAudioSessionTypeVideo) {
    ...
    if (self.outputFileType == TMCaptureAudioSessionTypeAudio || self.outputFileType == TMCaptureAudioSessionTypeVideo){
    ...
    if (filterImage) {
        ...
        @autoreleasepool {
            @synchronized(self) {
                ...
                if (!self.isStartWriting  && (self.outputFileType == TMCaptureAudioSessionTypeVideo || self.outputFileType == TMCaptureAudioSessionTypeSilentVideo)) {
                    //一个写入周期内只能执行一次
...
- (void)writingAudioSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection {
    @autoreleasepool {
        @synchronized(self) {
            if (!self.isStartWriting && self.outputFileType == TMCaptureAudioSessionTypeAudio) {
                [self.assetWriter startWriting];
                [self.assetWriter startSessionAtSourceTime:CMSampleBufferGetPresentationTimeStamp(sampleBuffer)];
                self.isStartWriting = YES;
            }
